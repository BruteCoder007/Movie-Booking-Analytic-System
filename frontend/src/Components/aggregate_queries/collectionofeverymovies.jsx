...
export default function Collectionofeverymovies(){
    const[results,setresult]=React.useState([]);
    const Datafetch=async ()=>{
    const res=await axios.post("http://localhost:5000/Aggregate/totalcollectionofmovie")
    ...
    return(<div>
     <Header text='Collection based on Movies' classname='header'/>
     <TableContainer component={Paper}>
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow >
            <TableCell align="right" style={{fontWeight:'900'}}>MovieID</TableCell>
            <TableCell align="right" style={{fontWeight:'900'}}>MovieName</TableCell>
        ...
        <TableBody>
          {results.map((result) => (
            ...
            >
              <TableCell component="th" scope="row" align='right'>{result._id.MovieID}</TableCell>
              <TableCell align="right">{result._id.MovieName}</TableCell>
